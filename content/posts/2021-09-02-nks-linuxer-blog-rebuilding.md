...
나는 대다수의 manifest를 shell 에서 그대로 적용해버리기때문에 $request\_method 이런 변수가 있는 부분은 \\$request\_method 역슬러시를 넣어서 평문 처리를해줬다.

\n\n\n\n

이 nginx conf configmaps 에서 특이점은 try\_files \\$uri \\$uri/ /index.php?\\$args; 부분이다. 이부분이 빠지면 wordpress 의 주소형식을 사용할수 없어 페이지 이동이 되지 않는다.
...
디비 복구후 wp-config 에서 define('DB\_HOST') 를 CDB로 변경했다. 기나긴 트러블 슈팅의 기간이 끝나가고 있었다.
